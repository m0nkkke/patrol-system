...
export default function ScheduleShopPickerScreen(): React.ReactElement {
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage,
  } = useInfiniteShops({ search: debouncedSearch, isActive: true, sort });

  const openShop = useCallback(
    (shop: Shop) => router.push({ pathname: '/schedules/[shopId]', params: { shopId: shop.id } }),
    [router],
  );
    ...
    <Screen padded={false}>
      <View style={styles.header}>
        <Header
          ...
          subtitle="Магазин, для которого настраиваете расписания"
          onBack={() => router.back()}
        />
        <TextField
          value={search}
      ...
      {isPending ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => <ShopCard shop={item} onPress={openShop} showStatus={false} />}
        />
      )}
    </Screen>
  );
