...
- Миграции не создают учётные записи и не содержат известных ключей. Первый production-администратор
  создаётся отдельной командой `admin:bootstrap:prod` из переменных окружения.
...
- Если refresh-токен ссылается на уже отсутствующего пользователя, `refresh` возвращает `401`, а
  audit-событие записывается с `userId = null`. `logout` остаётся идемпотентным и также пишет
  `userId = null`; исходный subject токена сохраняется только в `meta.tokenSubject`.
