...
- `GET /api/v1/mobile/shops/:shopId/route` — возвращает снимок маршрута активного обхода текущего СК в выбранном назначенном магазине.
...
- `GET /api/v1/mobile/route` — тот же снимок для основного назначенного магазина текущего СК.
...
## Снимок маршрута для offline

`POST /mobile/patrols/start` передаёт выбранный `routeId` в сервис обходов; если маршрут указан
в расписании, применяется он. Несовпадение явно выбранного маршрута и расписания отклоняется.
Ответ старта и `GET /mobile/patrols/active` содержат `routeSnapshot` — массив `PatrolSnapshotPoint`.
`GET /mobile/shops/:shopId/route` возвращает непосредственно этот массив (без обёртки).
Каждая точка содержит `id`, `shopId`, `name`, `isActive`, маршрутный `sortOrder`, обязательный
`dwellSeconds`, а также доступные `description`, `photoFileId`, `nfcTagId`, `nfcTag`.
В снимке присутствуют только точки выбранного маршрута, включая уже пройденные. Если обход
запущен без `routeId`, при старте фиксируются активные точки магазина с выдержкой 90 секунд.

Mobile должен сохранять снимок по `patrolId` после старта, до перехода offline. Для каждой
следующей точки берётся её `dwellSeconds`, включая 0 и 120; оба скана обязательны даже при 0.
Редактирование маршрута не меняет снимок уже начатого обхода. Снимок отражает NFC и описание
на момент старта; текущая активность точки и действительность NFC при сканировании по-прежнему
проверяются backend. Архивирование точки и замена метки не отменяются снимком.

Нет активного обхода в выбранном магазине — `MOBILE_ACTIVE_PATROL_REQUIRED` (400).
Магазин вне назначений — `MOBILE_SHOP_FORBIDDEN` (400). Для обходов, начатых до миграции
`1781782000000`, без снимка возвращается `PATROL_ROUTE_SNAPSHOT_UNAVAILABLE` (400):
для проверки нового сценария нужен новый обход. Подмена снимка текущими точками магазина исключена.
...
7. После последней точки маршрут считается настроенным. Магазин получает `route_status = ready` только после добавления активного расписания; до этого сохраняется `setup_in_progress`.
