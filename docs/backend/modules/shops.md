...
- Для роли `inspector` `GET /api/v1/shops` возвращает только магазины, назначенные этому проверяющему. Фильтры, поиск, сортировка и пагинация применяются внутри этой области доступа.
