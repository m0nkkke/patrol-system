...
Список включает активных и неактивных сотрудников и поддерживает фильтры `shopId`, `role`,
`isActive`, `search`, пагинацию и сортировку. Для `inspector` область ограничивается пересечением
назначенных магазинов. Ответ не содержит авторизационных данных, сессий, push-токенов и device ID.
