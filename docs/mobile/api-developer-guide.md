...
Ответ содержит только активные окна текущего дня, в которых `startTime <= текущее время < endTime`, и по которым в текущем окне еще нет неотмененного обхода. Поле `dueAt` возвращается в UTC и должно отображаться пользователю в локальном времени устройства.
